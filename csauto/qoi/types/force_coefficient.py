...
Workflow:
    1. `prepare` injects the rendered C++ template into caseXXXX/SRC/.
       The template integrates pressure and viscous forces over the named
       boundary at every time step and writes a CSV under monitoring/.
...
Note: the C++ template targets the code_saturne v9 user-file API. It is a
...
_CPP_TEMPLATE = Template(
    r"""/*============================================================================
 * cs_user_extra_operations.cpp — auto-generated by csauto (force_coefficient)
 *
 * Recipe boundary : ${boundary}
 * Output CSV      : ${output_csv}
 *
 * DO NOT EDIT — regenerated by csauto on `prepare`.
 *
 * Targets code_saturne >= 9.0 user-file API. The template integrates
 * pressure and viscous forces over the named boundary at every time step
 * and writes one row per step to the output CSV (rank 0 only).
 *============================================================================*/
...
def render_template(boundary: str) -> str:
    """Render the C++ user-file source for a given boundary."""
    return _CPP_TEMPLATE.substitute(
        boundary=boundary,
...
__all__ = ["ForceCoefficientExtractor", "output_csv_path", "render_template"]
